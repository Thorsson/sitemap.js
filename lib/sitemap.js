...
function SitemapItem(conf) {
  ...
  if (video) {
    ...
    if (video.thumbnailLoc) {
      this['video:video']['video:thumbnail_loc'] = video.thumbnailLoc;
    }

    if (video.title) {
      this['video:video'].title = video.title;
    }

    if (video.description) {
      this['video:video'].description = video.description;
    }

    if (video.playerLoc) {
      this['video:video']['video:player_loc'] = video.playerLoc;
    }

    this['video:video'] = this.videoFormatString();
  }
}
...
SitemapItem.prototype.toString = function () {
      // result xml
  var xml = '<url> {loc} {lastmod} {changefreq} {priority} {video:video}</url>'
      // xml property
    , props = ['loc', 'lastmod', 'changefreq', 'priority', 'video:video']
...
SitemapItem.prototype.videoFormatString = function() {
  var xml = _.map(this['video:video'], function(value, key) {
    return '<video:'+key+'>'+value+'</video:'+key+'>';
  });
